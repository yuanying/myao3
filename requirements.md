...
| Short-term Memory | 数時間〜数日 | 制限あり | 最近の会話、一時的な文脈 | SlackMessage DB |
| Long-term Memory | 永続 | 制限あり | 個人の特徴、重要な出来事、学習内容 | LLM Wiki（ファイルシステム） |

**Long-term Memory = LLM Wiki 2段階方式**:

```
ループ内（随時）
  → write_wiki_page("notes/YYYY-MM-DD", ...) で短期メモに追記
...
SQLModel ベースの DB は廃止。人物情報は `people/{canonical_slug}.md` が canonical source。
...
#### FR-MEMORY-005: 夜間記憶整理

| 項目 | 内容 |
|------|------|
| 概要 | 毎日深夜に notes/ を整理し、長期記憶ページと index.md を更新する |
| 優先度 | 高 |
| フェーズ | Phase 2 |
...
```
夜間整理フロー:
  1. notes/YYYY-MM-DD.md を読む
  2. 関連する long-term ページを読む
  3. 同一人物判定 → 必要ならページをマージ
  4. people/, topics/, communities/, self.md を更新
  5. index.md を最新状態に更新
```
...
| ストレージ (DB) | SQLite (Longhorn PVC) | Short-term Memory（会話履歴）の永続化 |
...
| 夜間整理 | 毎日深夜に notes/ から long-term ページと index.md を更新するプロセス |
