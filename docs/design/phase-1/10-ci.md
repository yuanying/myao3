...
| ジョブ名 | 説明 |
|---------|------|
| lint | ruff による lint チェック + ty による型チェック |
...
※ `uv sync` により editable install されるため、PYTHONPATH の設定は不要。
...
lint と test は並列実行され、フィードバックを早くする。
...
**期待結果:**
- lint ジョブの ty check ステップが成功する

### TC-10-004: lint ジョブの型チェック失敗
...
- [x] .github/workflows/ci.yaml が作成されている
- [x] push 時に CI が実行される
- [x] PR 時に CI が実行される
- [x] lint ジョブが ruff check と ruff format を実行する
- [x] lint ジョブが ty check を実行する
- [x] test ジョブが pytest を実行する
- [x] MOCK_LLM=true でテストが実行される
- [x] 全てのテストケースがパスする
