...
### TC-07-008: デフォルト設定ファイルでの起動

**手順:**
1. `python -m myao3` を引数なしで実行

**期待結果:**
- デフォルトの config.yaml を読み込む
- config.yaml が存在しない場合はエラー終了
...
- [x] `python -m myao3 --config config.yaml` で起動できる
- [x] 設定読み込み → ログ設定 → コンポーネント初期化の順で実行される
- [x] メインループがイベントを逐次処理する
- [x] SIGINT/SIGTERM でグレースフルシャットダウンする
- [x] シャットダウンタイムアウト（30秒）が機能する
- [x] 全てのテストケースがパスする
